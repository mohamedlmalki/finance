...
const handleFetchModuleFields = async (socket, data) => {
    ...
    try {
        console.log(`\n📡 [INVENTORY] Fetching fields for: ${moduleApiName}`);
        ...
        console.log(`✅ [INVENTORY] Success! Found ${fields.length} fields.`);
    } catch (error) {
        const parsed = parseError(error);
        console.error(`❌ [INVENTORY ERROR] ${parsed.message}`);
...
const handleStartBulkCustomJob = async (socket, data) => {
    ...
    console.log(`\n🧠 [INVENTORY SINGLE BATCH] Pushing account ${selectedProfileName} to Redis Queue...`);
...
const handleStartMasterBatch = async (socket, data) => {
    ...
    console.log(`\n🧠 [INVENTORY MASTER BATCH] Pushing ${payloads.length} accounts to Redis Queues...`);
