...
const initDB = async () => {
    try {
        ...
        console.error("❌ [DB ERROR] Failed to initialize PostgreSQL:", err.message);
    }
};
...
module.exports = {
    ...
    getAllJobs: async () => {
        // Fetches jobs and aggregates the counts instantly using Postgres's powerful engine
        const { rows } = await pool.query(`
            SELECT j.*, 
            (SELECT COUNT(*) FROM job_results WHERE jobId = j.id) as "processedCount",
