...
const spawnQueueProcessor = async (socket, data, isMaster = false) => {
    ...
    // 2. Setup the Job details (🚨 ADDED appendAccountName & accountIndex)
    let { webhookUrl, bulkData, staticData, bulkField, delay, selectedProfileName, activeProfile, trackingEnabled, targetHtmlField, startingRowNumber, stopAfterFailures, appendAccountName, accountIndex } = data;
        ...
        const jobs = rows.map((row, index) => {
            ...
            return {
                ...
                data: {
                    ...
                    appendAccountName, accountIndex // 🚨 PASSED TO WORKER
                },
...
const handleStartMasterBatch = async (socket, data) => {
    ...
    console.log(`\n🧠 [FLOW MASTER BATCH] Pushing ${payloads.length} accounts to Redis Queues...`);
