...
const axios = require('axios'); // Added for Flow
const db = require('./postgres');
const { emitToWeb } = require('./redis-pubsub');
const { makeApiCall, parseError } = require('./utils');

console.log("=====================================");
console.log("🚀 HEAVY WORKER NODE STARTED (PRODUCTION)");
console.log("=====================================");
...
// Flow specific tracking (Not Base64 encoded)
...
const processZohoJob = async (job) => {
    ...
    const checkAndFreeze = async (stage) => {
        ...
        if (status === 'paused') {
            console.log(`❄️ [FROZEN] Row ${rowNumber} for ${selectedProfileName} is held in stasis at stage: ${stage}...`);
            while (status === 'paused') {
            ...
            console.log(`🔥 [UNFROZEN] Row ${rowNumber} for ${selectedProfileName} resuming!`);
        }
...
// ----------------------------------------------------------------------------
// FLOW WEBHOOK PROCESSOR (NEWLY INTEGRATED)
// ----------------------------------------------------------------------------
const processFlowJob = async (job) => {
    const { rowData, identifier, webhookUrl, selectedProfileName, delay, activeProfile, rowNumber, trackingEnabled, targetHtmlField, bulkField, staticData, jobId, appendAccountName, accountIndex } = job.data;
    ...
    const checkAndFreeze = async (stage) => {
        ...
        if (status === 'paused' || status === 'paused_queued') {
            console.log(`❄️ [FROZEN] Flow Row ${rowNumber} for ${selectedProfileName} is held in stasis...`);
            ...
            console.log(`🔥 [UNFROZEN] Flow Row ${rowNumber} for ${selectedProfileName} resuming!`);
        }
        if (status === 'ended') return 'abort';
        return 'continue';
    };
    ...
    // 🚨 BACKEND SAFETY NET: Ensures exact 36 breaks & correct index without duplicating the Frontend's work
    if (appendAccountName && accountIndex) {
        ...
        if (rawPayload.description !== undefined && rawPayload.description !== null) {
            // Strips any potential typos/duplicates the user made, and enforces the perfect string
    ...
    else await new Promise(res => setTimeout(res, 100)); // Minimum 100ms throttle to protect DB

    const gapMs = Date.now() - startTime;
    const timeSinceLastTicket = isSuccess ? `${Math.floor(gapMs / 1000)}.${(gapMs % 1000).toString().padStart(3, '0')}s` : 'Error'; 
    ...
    // Auto-Pause & Retry Logic
    if (isSuccess) {
        await db.query("UPDATE jobs SET consecutivefailures = 0 WHERE id = $1 AND status = 'running'", [jobId]);
        emitToWeb('flowResult', { rowNumber, identifier, stage: 'complete', success: true, details, response: fullResponse, profileName: selectedProfileName, time: timeSinceLastTicket, timestamp: new Date() });
    } else {
        let shouldCooldown = false;
        let stopLimit = 0;
        let currentJob = await db.query("SELECT consecutivefailures, stopafterfailures FROM jobs WHERE id = $1", [jobId]);
        ...
        emitToWeb('flowResult', { rowNumber, identifier, stage: 'complete', success: false, details, response: fullResponse, profileName: selectedProfileName, time: timeSinceLastTicket, timestamp: new Date() });
...
// ============================================================================
// 🕵️ THE BOUNCER (Concurrency, Ordering, & Timers)
// ============================================================================
setInterval(async () => {
    try {
        ...
        // 🟢 FIX: Handle mapping active queues for both Custom Modules AND Flow
        ...
        for (const dbJob of sortedJobs) {
            ...
            // 🟢 FIX: Set correct Queue name based on the Job Type
            ...
            if (dbJob.status === 'running' && !activeWorkers[queueName]) {
                ...
                console.log(`▶️ [STARTING] Account: ${dbJob.profilename} (Module: ${dbJob.jobtype}) | Speed: ${rowConcurrency}`);
                
                const worker = new Worker(queueName, async (job) => {
                    
                    // 🟢 FIX: Route Flow Jobs to the new Flow Webhook Processor
                    if (dbJob.jobtype.startsWith('Flow_')) {
                    ...
                    // Standard Zoho Routing
                    let serviceType = 'inventory'; 
                    if (job.data.activeProfile?.billing?.customModuleApiName === job.data.actualTargetModule) serviceType = 'billing';
                    else if (job.data.activeProfile?.inventory?.customModuleApiName === job.data.actualTargetModule) serviceType = 'inventory';
                    else if (job.data.activeProfile?.billing?.orgId && !job.data.activeProfile?.inventory?.orgId) serviceType = 'billing';
                ...
                worker.on('drained', async () => {
                    ...
                    if (checkDb.rows.length > 0) {
                        ...
                        if (currentStatus === 'paused') {
                            console.log(`⏸️ [PAUSED] Account: ${dbJob.profilename} (Worker drained safely)`);
                            await worker.close();
                            delete activeWorkers[queueName];
                            return; 
                        }
                        if (currentStatus === 'ended') {
                            console.log(`🛑 [ENDED] Account: ${dbJob.profilename} (Worker drained safely)`);
                            await worker.close();
                            delete activeWorkers[queueName];
                            return; 
                        }
                    ...
                    console.log(`✅ [FINISHED] Account: ${dbJob.profilename}`);
