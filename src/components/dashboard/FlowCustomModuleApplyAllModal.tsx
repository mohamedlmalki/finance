...
interface FlowCustomModuleApplyAllModalProps {
  ...
  initialData: {
      ...
      appendAccountName: boolean; // 🚨 Added
  };
}
...
export const FlowCustomModuleApplyAllModal: React.FC<FlowCustomModuleApplyAllModalProps> = ({ 
  ...
  const [appendAccountName, setAppendAccountName] = useState(false); // 🚨 Added
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setAppendAccountName(initialData.appendAccountName || false); // 🚨 Added
  ...
  }, [isOpen, initialData, eligibleProfiles]);
  ...
  const handleApply = () => {
    onApply(
        ...
        { trackingEnabled, targetHtmlField, appendAccountName }, // 🚨 Added
        applyOptions 
    );
    onClose();
  };
  ...
  const isApplyDisabled = selected.length === 0 || 
                          ...
                          (applyOptions.iterator && !bulkData) ||
