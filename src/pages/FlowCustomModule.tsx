...
const FlowCustomModule: React.FC<FlowCustomModuleProps> = ({ 
    ...
    const [statusFilter, setStatusFilter] = useState<'all' | 'success' | 'waiting' | 'error'>('all');
        ...
        const handleDbSearch = (data: any) => { 
            if (data.profileName === activeProfileName) {
                if (isAutoLoadingRef.current[activeProfileName]) {
                    ...
                    setJobs(prev => { const j = getSafeJob(prev, data.profileName); const sortedHistory = [...data.results].map(r => ({ ...r, stage: r.success === null ? 'waiting' : 'complete' })).sort((a, b) => (b.rowNumber || 0) - (a.rowNumber || 0)); if (j.results.length === 0) return { ...prev, [data.profileName]: { ...j, results: sortedHistory } }; return prev; });
                } else {
                    setIsSearchingDB(false); setDbSearchResults(data.results.map((r:any) => ({ ...r, stage: r.success === null ? 'waiting' : 'complete' }))); toast({ title: "Database Search Complete", description: `Found ${data.results.length} items.` }); 
                }
            } 
        };
                            ...
                            <CardHeader className="pb-4">
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex items-center space-x-3 select-none">
                                        ...
                                        <Badge variant="outline" className={cn("bg-amber-500/10 text-amber-600 cursor-pointer border-transparent", statusFilter === 'waiting' && "ring-2 ring-amber-500")} onClick={() => setStatusFilter(statusFilter === 'waiting' ? 'all' : 'waiting')}><Loader2 className="h-3 w-3 mr-1 animate-spin" /> {results.filter(r => r.stage === 'waiting').length} Waiting</Badge>
                                    ...
                                    <table className="w-full">
                                        ...
                                        <tbody className="bg-card divide-y divide-border">
                                            {(jobState as any)._isQueued ? (<tr><td colSpan={5} className="p-8 text-center text-amber-600 font-medium">Waiting for previous accounts to finish before starting...</td></tr>) : paginatedResults.length === 0 ? (<tr><td colSpan={5} className="p-8 text-center text-muted-foreground">No results.</td></tr>) : (paginatedResults.map((result: any) => (
                                                <tr key={result.rowNumber} className={cn("transition-colors duration-300", result.stage === 'complete' && !result.success ? 'bg-destructive/5' : (result.stage === 'waiting' ? 'bg-amber-500/5' : ''))}>
                                                    ...
                                                    <td className={cn("p-3 text-sm font-medium", result.stage === 'complete' && !result.success ? 'text-destructive' : (result.stage === 'waiting' ? 'text-amber-600 animate-pulse' : 'text-muted-foreground'))}>{result.details}</td>
                                                    ...
                                                    <td className="p-3 text-center">
                                                        <Dialog>
                                                            <DialogTrigger asChild><Button variant="ghost" size="sm" className="h-8 px-3 border shadow-sm transition-all hover:scale-105">{result.stage === 'waiting' ? <Loader2 className="h-4 w-4 text-amber-500 animate-spin" /> : (result.success ? <CheckCircle2 className="h-4 w-4 text-green-600" /> : <XCircle className="h-4 w-4 text-red-600" />)}<Eye className="h-3 w-3 text-muted-foreground ml-2" /></Button></DialogTrigger>
