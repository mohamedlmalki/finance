...
const BillingCustomModule: React.FC<BillingCustomModuleProps> = ({ 
    ...
    const billingProfiles = useMemo(() => {
        return profiles.filter(p => p.billing?.orgId && p.billing.orgId.trim() !== '');
    }, [profiles]);
    ...
    const selectedProfile = useMemo(() => {
        return billingProfiles.find(p => p.profileName === activeProfileName) || null;
    }, [billingProfiles, activeProfileName]);  
        ...
        const handleSync = (allJobs: any[]) => {
            setJobs(prev => {
                ...
                allJobs.forEach(dbJob => {
                    // 🛠️ THE BUG FIX: Postgres returns lowercase 'jobtype'. We must check both!
                    const jt = dbJob.jobType || dbJob.jobtype;
                    if (!jt || !jt.startsWith('bil_')) return;

                    const rawModule = jt.replace(/^bil_/, '');
        ...
        const handleJobStarted = (data: any) => {
            ...
            setJobs(prev => {
                ...
                return { 
                    ...
                    [data.profileName]: { 
                        ...profileJob, 
                        formData: { ...profileJob.formData, moduleApiName: rawModule },
                        _isQueued: false, 
                        isProcessing: true, 
        ...
        const handleJobCleared = (data: any) => {
            ...
            setJobs(prev => {
                ...
                return { 
                    ...
                    [data.profileName]: { 
                        ...profileJob, 
                        formData: { ...profileJob.formData, moduleApiName: rawModule, bulkData: '' },
    ...
    useEffect(() => {
        ...
        const handleAllJobsCleared = (data: any) => {
            if (data.jobType && data.jobType.startsWith('bil_cm_')) {
                setJobs(prev => {
                    const next = { ...prev };
            ...
            else if (data.jobType === `bil_${expectedJobType}`) {
                setJobs(prev => {
                    const next = { ...prev };
                    Object.keys(next).forEach(profile => {
    ...
    const handleFetchFields = (forceUpdate = false) => {
        ...
        socket.emit('fetchBillingModuleFields', { selectedProfileName: selectedProfile.profileName, moduleApiName: moduleName, activeProfile: selectedProfile });
    ...
    const handleClearJob = () => { if (!activeProfileName || !formData.moduleApiName || !socket) return; if (window.confirm(`⚠️ Are you sure you want to completely WIPE all data for ${activeProfileName}?`)) { socket.emit('clearJob', { profileName: activeProfileName, jobType: `bil_${formData.moduleApiName}` }); } };
    const handleClearAllJobs = () => { 
        if (!socket) return; 
        if (window.confirm(`🚨 DANGER: Are you sure you want to completely WIPE ALL databases for EVERY profile on Billing Custom Modules?`)) { 
            
            // 🛠️ THE FIX: Loop through EVERY profile and wipe its specific API name individually
            billingProfiles.forEach(p => {
                const modName = p.billing?.customModuleApiName || formData.moduleApiName;
                if (modName && modName.trim() !== '') {
                    socket.emit('clearJob', { profileName: p.profileName, jobType: `bil_${modName}` });
                }
            });
            ...
            setJobs(prev => {
                ...
                Object.keys(next).forEach(profile => {
                    next[profile] = { 
                        ...next[profile], 
                        results: [], totalToProcess: 0, isProcessing: false, isPaused: false, isComplete: false, 
                        formData: { ...next[profile].formData, bulkData: '' }, 
                        processedCount: 0, successCount: 0, errorCount: 0, 
                        _forceStopped: true, _isQueued: false 
                    } as any;
                });
            ...
            toast({ title: "Master Wipe Initiated", description: "All Billing databases are being cleared." });
        } 
    };
    ...
    const handleApplyAll = (selectedProfileNames: string[], modalFormData: any, modalTrackingData: any, applyOptions: any = { iterator: true, staticFields: true, execution: true, tracking: true }) => {
        ...
        setJobs(prev => {
            ...
            profilesToUpdate.forEach(profileName => {
                ...
                if (!targetFields) {
                    socket?.emit('fetchBillingModuleFields', { selectedProfileName: profileName, moduleApiName: targetModName, activeProfile: billingProfiles.find(p => p.profileName === profileName) });
                }
                ...
                if (applyOptions.staticFields || applyOptions.tracking) {
                    const targetAccIndex = billingProfiles.findIndex(p => p.profileName === profileName) + 1;
    ...
    const handlePauseResume = () => {
        ...
        const exactActiveModule = `bil_${currentJob.formData?.moduleApiName || billingProfiles.find(p => p.profileName === activeProfileName)?.billing?.customModuleApiName || formData.moduleApiName}`;
    ...
    const handleMasterStopAll = () => {
        ...
        setJobs(prev => {
            const next = { ...prev };
            activeProfiles.forEach(p => {
                if (next[p.profileName]) {
                    next[p.profileName] = { ...next[p.profileName], isProcessing: false, isPaused: false, isComplete: true, _forceStopped: true, _isQueued: false } as any;
                }
            });
            return next;
        });
                                                    ...
                                                    {(jobState.isProcessing || (jobState as any)._isQueued) && (
                                                        <>
                                                            ...
                                                            <Button size="sm" variant="destructive" className="flex-1" onClick={() => { 
                                                                const exactActiveModule = `bil_${jobs[activeProfileName]?.formData?.moduleApiName || billingProfiles.find(p => p.profileName === activeProfileName)?.billing?.customModuleApiName || formData.moduleApiName}`;
                            ...
                            <div className="pt-6 border-t space-y-4 mt-6">
                                ...
                                <div className="space-y-3 pt-2">
                                    <div className="flex items-center justify-between">
                                        <Label className="text-xs font-bold text-muted-foreground uppercase">All Accounts ({eligibleProfilesCount})</Label>
                                        <div className="flex items-center gap-2">
                                    ...
                                    <div className="flex gap-2">
                                        <Button size="sm" className="flex-1" onClick={handleMasterBatchStart} disabled={activeMasterJobs.totalProcessing > 0 || eligibleProfilesCount === 0 || isStartingBatch}>
                                            {isStartingBatch ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <Play className="h-4 w-4 mr-2" />} {isStartingBatch ? "Syncing..." : `Start All`}
                                        </Button>
