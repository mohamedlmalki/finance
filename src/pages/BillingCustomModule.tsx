// --- FILE: src/pages/BillingCustomModule.tsx ---
...
const BillingCustomModule: React.FC<BillingCustomModuleProps> = ({ 
    ...
    const [activeProfileName, setActiveProfileName] = useState<string | null>(() => {
        return localStorage.getItem('billingCustomModuleActiveProfile') || null;
    });
    ...
    const [masterBatchConcurrency, setMasterBatchConcurrency] = useState(() => {
        return Number(localStorage.getItem('bilMasterBatchConcurrency')) || 1;
    });
    ...
    const billingProfiles = profiles.filter(p => p.billing?.orgId && p.billing.orgId.trim() !== '');
    
    useEffect(() => {
        if (activeProfileName) localStorage.setItem('billingCustomModuleActiveProfile', activeProfileName);
        localStorage.setItem('bilMasterBatchConcurrency', masterBatchConcurrency.toString());
    }, [activeProfileName, masterBatchConcurrency]);

    useEffect(() => {
        try {
            const savedForms = localStorage.getItem('billingCustomModuleForms_v1');
            if (savedForms) {
    ...
    useEffect(() => {
        try {
            ...
            if (hasData) localStorage.setItem('billingCustomModuleForms_v1', JSON.stringify(formsToSave));
        } catch (error) {}
    }, [jobs]);
    ...
    useEffect(() => {
        ...
        socket.on('jobPaused', (data) => console.log(`📥 [SOCKET FRONTEND] Received 'jobPaused' for ${data.profileName}`, data));
        socket.on('jobStarted', (data) => console.log(`📥 [SOCKET FRONTEND] Received 'jobStarted' for ${data.profileName}`, data));
        socket.on('bulkComplete', (data) => console.log(`📥 [SOCKET FRONTEND] Received 'bulkComplete' for ${data.profileName}`, data));
        socket.on('bulkEnded', (data) => console.log(`📥 [SOCKET FRONTEND] Received 'bulkEnded' for ${data.profileName}`, data));
        ...
        const handleSync = (allJobs: any[]) => {
            setJobs(prev => {
                ...
                const ignoredJobTypes = ['invoice', 'books', 'books-contact', 'expense', 'inventory', 'billing-contact', 'billing-invoice'];

                allJobs.forEach(dbJob => {
                    if (ignoredJobTypes.includes(dbJob.jobType)) return;
                    if (dbJob.jobType?.startsWith('Flow_')) return;

                    const existingJob = next[dbJob.profileName] || createInitialJobState();
                    const validModule = dbJob.jobType; 
                    ...
                    if (validModule) {
                        ...
                        if (
                            ...
                            next[dbJob.profileName] = {
                                ...
                                successCount: newSuccess,
                                errorCount: newError,
                                processingStartTime: dbJob.processingstarttime ? new Date(dbJob.processingstarttime) : existingJob.processingStartTime
                            };
                            hasChanges = true;
                        }
                    }
        ...
        const handleJobStarted = (data: any) => {
            setJobs(prev => {
                ...
                return { 
                    ...
                    [data.profileName]: { 
                        ...profileJob, 
                        formData: { ...profileJob.formData, moduleApiName: data.jobType },
                        _isQueued: false, 
                        isProcessing: true, 
            ...
            setJobs(prev => {
                ...
                return { 
                    ...
                    [data.profileName]: { 
                        ...profileJob, 
                        formData: { ...profileJob.formData, moduleApiName: data.jobType },
                        isProcessing: false, isPaused: false, isComplete: true, _forceStopped: true, _isQueued: false 
                    } as any 
                };
            });
        };
        ...
        const handleJobCleared = (data: any) => {
            setJobs(prev => {
                ...
                return { 
                    ...
                    [data.profileName]: { 
                        ...profileJob, 
                        formData: { ...profileJob.formData, moduleApiName: data.jobType, bulkData: '' },
                        results: [], totalToProcess: 0, isProcessing: false, isPaused: false, isComplete: false, processedCount: 0, successCount: 0, errorCount: 0, _forceStopped: true, _isQueued: false 
                    } as any 
                };
            });
        ...
        socket.on('billingCustomModuleResult', handleBufferedResult);
    ...
    useEffect(() => {
        ...
        const expectedJobType = selectedProfile.billing?.customModuleApiName;
        ...
        const handleFieldsResult = (res: any) => {
            ...
            const targetProfileObj = billingProfiles.find(p => p.profileName === profileToUpdate);
        ...
        const handleAllJobsCleared = (data: any) => {
            if (data.jobType && data.jobType.startsWith('cm_')) {
                setJobs(prev => {
                    const next = { ...prev };
            ...
            else if (data.jobType === expectedJobType) {
                setJobs(prev => {
                    const next = { ...prev };
                    Object.keys(next).forEach(profile => {
    ...
    }, [socket, activeProfileName, selectedProfile, setJobs, createInitialJobState, isStartingBatch, billingProfiles]);
    ...
    const handleClearJob = () => { if (!activeProfileName || !formData.moduleApiName || !socket) return; if (window.confirm(`⚠️ Are you sure you want to completely WIPE all data for ${activeProfileName}?`)) { socket.emit('clearJob', { profileName: activeProfileName, jobType: formData.moduleApiName }); } };
    const handleClearAllJobs = () => { if (!formData.moduleApiName || !socket) return; if (window.confirm(`🚨 DANGER: Are you sure you want to completely WIPE ALL databases for EVERY profile on module ${formData.moduleApiName}?`)) { socket.emit('clearAllJobs', { jobType: formData.moduleApiName }); } };
    ...
    const eligibleProfilesCount = useMemo(() => {
        return billingProfiles.filter(p => {
    ...
    const handleStartJob = () => {
        ...
        const accountIndex = billingProfiles.findIndex(p => p.profileName === activeProfileName) + 1;
    ...
    const handleApplyAll = (selectedProfileNames: string[], modalFormData: any, modalTrackingData: any, applyOptions: any = { iterator: true, staticFields: true, execution: true, tracking: true }) => {
        ...
        setJobs(prev => {
            ...
            profilesToUpdate.forEach(profileName => {
                ...
                if (!targetFields) {
                    socket?.emit('fetchModuleFields', { selectedProfileName: profileName, moduleApiName: targetModName, activeProfile: billingProfiles.find(p => p.profileName === profileName) });
                }
    ...
    // 🚀 THE FIX: EXACT SINGLE MODULE LOOKUP FOR SINGLE BUTTONS
    const handlePauseResume = () => {
        if (!socket || !activeProfileName) return;
        const currentJob = jobs[activeProfileName];
        if (!currentJob) return;

        const exactActiveModule = currentJob.formData?.moduleApiName || billingProfiles.find(p => p.profileName === activeProfileName)?.billing?.customModuleApiName || formData.moduleApiName;
        console.log(`[FRONTEND SINGLE] Pause/Resume clicked for ${activeProfileName}, Module: ${exactActiveModule}`);

        if (currentJob.isPaused) {
            socket.emit('resumeJob', { profileName: activeProfileName, jobType: exactActiveModule });
            setJobs(prev => ({ ...prev, [activeProfileName]: { ...prev[activeProfileName], isPaused: false, isProcessing: !(prev[activeProfileName] as any)._isQueued, _forcePaused: false, _forceStopped: false } as any }));
    ...
    }, [jobs, billingProfiles]);

    const handleMasterPauseAll = () => {
        console.log("\n=================================================");
        console.log("🔴 [FRONTEND GLOBAL] 'Pause All' Button Clicked!");
        
        const profilesToPause = billingProfiles.filter(p => {
            const job = jobs[p.profileName];
            const hasValidModule = !!p.billing?.customModuleApiName && p.billing.customModuleApiName !== 'cm_';
            const isRunning = job?.isProcessing && !job?.isPaused;
            const isWaiting = !!(job as any)?._isQueued;
            return hasValidModule && isRunning && !isWaiting;
        });

        console.log("🔴 [FRONTEND] Sending pauseJob socket command to:", profilesToPause.map(p => p.profileName));

        let pausedCount = 0;
        profilesToPause.forEach(p => {
            const targetModName = jobs[p.profileName]?.formData?.moduleApiName || p.billing?.customModuleApiName;
            ...
            socket?.emit('pauseJob', { profileName: p.profileName, jobType: targetModName }); 
            pausedCount++;
        });
        toast({ title: "Global Batch Paused", description: `Paused ${pausedCount} active accounts.` });
    };

    const handleMasterForceResume = () => {
        console.log("\n=================================================");
        console.log("🟢 [FRONTEND GLOBAL] 'Resume All' Button Clicked!");
        
        const pausedProfiles = billingProfiles.filter(p => {
            const job = jobs[p.profileName];
            const hasValidModule = !!p.billing?.customModuleApiName && p.billing.customModuleApiName !== 'cm_';
            return hasValidModule && (job?.isPaused || (job as any)?._forcePaused) && !(job as any)?._isQueued;
        });
        ...
        pausedProfiles.forEach(p => {
            ...
            socket?.emit('resumeJob', { profileName: p.profileName, jobType: targetModName }); 
            resumedCount++;
        });
        toast({ title: "Global Batch Resumed", description: `Resumed ${resumedCount} accounts.` });
    };
    ...
    const handleMasterStopAll = () => {
        const activeProfiles = billingProfiles.filter(p => {
            const job = jobs[p.profileName];
            const hasValidModule = !!p.billing?.customModuleApiName && p.billing.customModuleApiName !== 'cm_';
            return hasValidModule && (job?.isProcessing || (job as any)?._isQueued);
        });
        
        activeProfiles.forEach(p => {
            const exactActiveModule = jobs[p.profileName]?.formData?.moduleApiName || p.billing?.customModuleApiName;
            socket?.emit('endJob', { profileName: p.profileName, jobType: exactActiveModule });
        ...
        toast({ title: "Global Batch Stopped", description: "All active jobs ended." });
    };

    const handleMasterBatchStart = async () => {
        console.log("\n=================================================");
        console.log("🚀 [FRONTEND GLOBAL] 'Start All' Button Clicked!");
        ...
        if (configuredProfiles.length === 0) return toast({ title: "Cannot Start Batch", description: "No accounts configured for custom modules.", variant: "destructive" });
        ...
        idleProfiles.forEach(profileData => {
            ...
            const sourceFields = formData.availableFields || [];
            ...
            if (profileName === activeProfileName) {
                ...
                if (sourceFields.length > 0) {
                    const { mappedStatic, mappedBulk, mappedTargetHtml } = performSmartMapping(sourceFields, targetFields, formData.staticData, formData.bulkField, (formData as any).targetHtmlField || '');
                    safeStaticData = mappedStatic; safeBulkField = mappedBulk; safeTargetHtmlField = mappedTargetHtml; 
                } else {
                    safeStaticData = formData.staticData || {}; safeBulkField = formData.bulkField || ''; safeTargetHtmlField = (formData as any).targetHtmlField || '';
                }
            }
            ...
            payloads.push({
                ...
                masterConcurrency: masterBatchConcurrency
            });
        });
        ...
        console.log(`🚀 [FRONTEND] Starting ${payloads.length} total modules globally!`);
            ...
            payloads.forEach(p => {
                ...
                next[p.selectedProfileName] = {
                    ...
                    formData: {
                        ...
                        moduleApiName: p.moduleApiName, 
                        bulkData: p.bulkData,
                        bulkField: p.bulkField
                    },
        ...
        socket?.emit(
            formData.moduleApiName?.includes('Billing') ? 'startMasterBulkBillingCustomJob' : 'startMasterBatchCustomJob', {
            moduleApiName: formData.moduleApiName, 
            concurrency: masterBatchConcurrency,
            payloads: payloads
        });
        
        toast({ title: "Global Batch Running", description: `Sent ${payloads.length} accounts to the background server.` });
    };
    ...
    const handleDatabaseSearch = () => { if (!activeProfileName || !socket || !filterText) return; setIsSearchingDB(true); socket.emit('startBulkBillingCustomJob', { isSearchRequest: true, selectedProfileName: activeProfileName, moduleApiName: formData.moduleApiName, query: filterText }); };
    const handleFullExportDB = () => { if (!activeProfileName || !socket) return; setIsExporting(true); socket.emit('startBulkBillingCustomJob', { isExportRequest: true, selectedProfileName: activeProfileName, moduleApiName: formData.moduleApiName }); };
    const handleManualVerify = () => { if (activeProfileName && socket) { setApiStatus({ status: 'loading', message: 'Re-checking connection...' }); socket.emit('checkApiStatus', { selectedProfileName: activeProfileName, service: 'billing' }); } };
    ...
    if (isWiping) {
        return (
            <DashboardLayout
                title="Billing Custom Modules"
                description="Push data to Zoho Subscriptions/Billing Custom Modules."
                service="billing"
    ...
    return (
        <>
            <DashboardLayout onAddProfile={onAddProfile} profiles={billingProfiles} selectedProfile={selectedProfile} onProfileChange={setActiveProfileName} jobs={jobs} socket={socket} onEditProfile={onEditProfile} onDeleteProfile={onDeleteProfile} service="billing" apiStatus={apiStatus} onManualVerify={handleManualVerify} onShowStatus={() => setIsStatusModalOpen(true)}>
                <div className="space-y-6">
                    <Card className="shadow-medium">
                        <CardHeader>
                            <CardTitle className="flex items-center space-x-2"><Database className="h-5 w-5 text-primary" /><span>Billing Custom Module Manager</span></CardTitle>
                        ...
                        <CardContent className="space-y-6">
                            ...
                            {!formData.moduleApiName && (<Alert variant="destructive"><AlertCircle className="h-4 w-4" /><AlertTitle>Configuration Required</AlertTitle><AlertDescription>This Billing account does not have a <strong>Custom Module API Name</strong> set. Please click the <strong>Settings (Gear Icon)</strong> next to the profile name to configure it.</AlertDescription></Alert>)}
                            
                            {isFetchingFields && (<div className="flex flex-col justify-center items-center py-12 text-muted-foreground bg-muted/20 rounded-lg border border-dashed"><Loader2 className="animate-spin h-8 w-8 mb-4 text-primary" /><span className="text-sm font-medium">Fetching latest fields from Zoho Billing...</span></div>)}
                                                    ...
                                                    {(jobState.isProcessing || (jobState as any)._isQueued) && (
                                                        <>
                                                            ...
                                                            <Button size="sm" variant="destructive" className="flex-1" onClick={() => { 
                                                                const exactActiveModule = jobs[activeProfileName]?.formData?.moduleApiName || billingProfiles.find(p => p.profileName === activeProfileName)?.billing?.customModuleApiName || formData.moduleApiName;
            ...
            <CustomModuleApplyAllModal isOpen={isApplyAllModalOpen} onClose={() => setIsApplyAllModalOpen(false)} onApply={handleApplyAll} profiles={billingProfiles} moduleApiName={formData.moduleApiName} availableFields={formData.availableFields || []} service="billing" initialData={{ bulkField: formData.bulkField, bulkData: formData.bulkData, staticData: formData.staticData, delay: formData.delay, concurrency: formData.concurrency, stopAfterFailures: formData.stopAfterFailures, trackingEnabled: trackingEnabled, targetHtmlField: (formData as any).targetHtmlField || '', appendAccountName: (formData as any).appendAccountName || false }} />
        </>
    );
};
