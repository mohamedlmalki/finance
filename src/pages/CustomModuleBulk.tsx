...
const CustomModuleBulk: React.FC<CustomModuleBulkProps> = ({ 
    ...
    const inventoryProfiles = profiles.filter(p => p.inventory?.orgId && p.inventory.orgId.trim() !== '');
    ...
    useEffect(() => {
        ...
        const handleSync = (allJobs: any[]) => {
            setJobs(prev => {
                ...
                const ignoredJobTypes = ['invoice', 'books', 'books-contact', 'expense', 'billing', 'billing-contact', 'billing-invoice'];
    ...
    const handlePauseResume = () => {
        ...
        const exactActiveModule = currentJob.formData?.moduleApiName || inventoryProfiles.find(p => p.profileName === activeProfileName)?.inventory?.customModuleApiName || formData.moduleApiName;
    ...
    const handleMasterPauseAll = () => {
        ...
        const profilesToPause = inventoryProfiles.filter(p => {
            const job = jobs[p.profileName];
            const hasValidModule = !!p.inventory?.customModuleApiName && p.inventory.customModuleApiName !== 'cm_';
        ...
        profilesToPause.forEach(p => {
            const targetModName = jobs[p.profileName]?.formData?.moduleApiName || p.inventory?.customModuleApiName;
    ...
    const handleMasterStopAll = () => {
        ...
        activeProfiles.forEach(p => {
            const exactActiveModule = jobs[p.profileName]?.formData?.moduleApiName || p.inventory?.customModuleApiName;
